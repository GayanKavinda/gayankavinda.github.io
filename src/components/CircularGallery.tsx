...
class App {
  ...
  onTouchUp(e: any) {
    ...
    if (distance < 5 && this.onClick) {
      const rect = this.container.getBoundingClientRect();
      this.mouse.set(
        2.0 * ((x - rect.left) / this.container.offsetWidth) - 1.0,
        2.0 * (1.0 - (y - rect.top) / this.container.offsetHeight) - 1.0
      );
      ...
      if (hits.length > 0) {
        ...
        const media = this.medias.find((m: any) => m.plane === hitMesh);
        if (media) {
          // Pass index relative to original items (not duplicated list)
          const originalLength = Math.floor(this.mediasImages.length / 2);
          const originalIdx = media.index % originalLength;
          this.onClick(media.image, originalIdx);
        }
      }
    }
