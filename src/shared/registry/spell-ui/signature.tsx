...
export function Signature({
  ...
  const height = fontSize * 2; // Increased to 2x fontSize to accommodate all large descenders like 'y'
  const horizontalPadding = fontSize * 0.2;
  const topMargin = fontSize * 0.4;
  const baseline = topMargin + fontSize;
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        let font;
        const fontPaths = [
          "https://unpkg.com/@fontsource/dancing-script/files/dancing-script-latin-400-normal.woff",
        ];
